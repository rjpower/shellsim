...
descriptor readiness. Timeout-bearing operations register a virtual event and wait on typed
child-or-deadline conditions. Command substitutions in
...
Callbacks made from within compound native operations still execute through nested Rust calls, so
suspension is not yet available at every bytecode instruction. Direct `time.sleep` calls register
a typed timer wait. Retryable native calls can retain normalized arguments on the bytecode frame;
`Popen.wait()` uses that path to retry after its child wakes the process.
...
waits, communication, stream I/O, and virtual deadlines now leave the VM stack and block the
owning logical process. Command
...
those callbacks. The nested scheduler helper remains only for the explicitly synchronous
`run_python` embedding API; scheduler-owned Python command execution does not use it. Deep or
...
3. The coherent Git baseline now covers human and porcelain status including compact branch
   headers, staged and working-tree diff, name-only, summary-stat, and whitespace-checking diff,
   bounded history, show, refs, branch
   creation/listing/deletion, checkout/switch, restore, reset, add, commits, tracked-file queries,
   tracked file moves/removals, and repository-local config. Repository state stays VFS-only;
   merges, remotes, and network operations remain explicitly unsupported.
